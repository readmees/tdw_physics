...
class Collision(Controller):
    ...
    def collision_force(self, commands):
        num_objects = 3 #TODO change back to random.choice([2, 3])
        ...
        # random.shuffle(self.records) #TODO uncommend
        # doesn't work 1, 2, 3, 4
        working_object = ['rh10']
        # Add 2-3 objects.
        for i in range(num_objects):
            i = 5
            o_id = Controller.get_unique_id()
            record = self.records[self.model_num ]
            ...
            print(self.records[self.model_num ].name)
            commands.extend(self.get_add_physics_object(model_name=self.records[self.model_num ].name,
...
# 1-10 doesn't work
    ...
    call(["ffmpeg",
        ...
        "-vcodec", "libx264",
        "-pix_fmt", "yuv420p",
        f"{path}coPllision/"+"image_only_video.mp4"])